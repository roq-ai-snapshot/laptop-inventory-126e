...
import { useRoqClient, useTransactionFindManyWithCount } from 'lib/roq';
...
export function TransactionListPage(props: TransactionListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useTransactionFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.transaction.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    { id: 'transaction_type', header: 'Transaction Type', accessorKey: 'transaction_type' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.transaction_date ? format(record?.transaction_date, 'dd-MM-yyyy') : '',
    },
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('laptop', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'laptop',
          header: 'Laptop',
          accessorKey: 'laptop',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/laptops/view/${record.laptop?.id}`}>
              {record.laptop?.model}
            </Link>
          ),
        }
