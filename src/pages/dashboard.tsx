...
import { useRoqPlatformUserProfiles, useInventoryCount, useLaptopCount, useUserCompanyCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const inventoryStat = useInventoryCount();

  const laptopStat = useLaptopCount();

  const userCompanyStat = useUserCompanyCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Inventories',
      result: inventoryStat?.data,
      isLoading: inventoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Laptops',
      result: laptopStat?.data,
      isLoading: laptopStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'User Companies',
      result: userCompanyStat?.data,
      isLoading: userCompanyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
