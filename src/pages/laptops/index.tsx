...
import { useRoqClient, useLaptopFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<LaptopInterface, unknown>;

interface LaptopListPageProps {
...
export function LaptopListPage(props: LaptopListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: [, 'inventory.count', 'transaction.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['model.contains', 'manufacturer.contains', 'serial_number.contains', 'status.contains'],
          ...params.filters,
        },
        'laptop',
      ),
  ...
  const { data, error, isLoading, mutate } = useLaptopFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.laptop.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: LaptopInterface) => {
    if (hasAccess('laptop', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/laptops/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'serial_number', header: 'Serial Number', accessorKey: 'serial_number' },
    {
      id: 'purchase_date',
      header: 'Purchase Date',
      accessorKey: 'purchase_date',
      cell: ({ row: { original: record } }: any) =>
        record?.purchase_date ? format(record?.purchase_date, 'dd-MM-yyyy') : '',
    },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.warranty_expiration ? format(record?.warranty_expiration, 'dd-MM-yyyy') : '',
    },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/laptops/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('laptop', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/laptops/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('laptop', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="laptop" />
        </Flex>

        {hasAccess('laptop', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/laptops/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
