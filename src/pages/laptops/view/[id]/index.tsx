...
import { useLaptopFindFirst, useRoqClient } from 'lib/roq';
...
function LaptopViewPage() {
  ...
  const { data, error, isLoading } = useLaptopFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('laptop', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/laptops/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Serial Number" text={data?.serial_number} />

                <FormListItem
                  label="Purchase Date"
                  text={data?.purchase_date ? format(data?.purchase_date, 'dd-MM-yyyy') : ''}
                />

                <FormListItem
                  label="Warranty Expiration"
                  text={data?.warranty_expiration ? format(data?.warranty_expiration, 'dd-MM-yyyy') : ''}
                />
