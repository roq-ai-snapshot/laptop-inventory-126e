...
function InventoryCreatePage() {
  ...
  const formik = useFormik<InventoryInterface>({
    initialValues: {
      quantity: 0,
      location: '',
      last_inventory_check: new Date(new Date().toDateString()),
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="last_inventory_check" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Last Inventory Check
            </FormLabel>
            <DatePicker
              selected={formik.values?.last_inventory_check ? new Date(formik.values?.last_inventory_check) : null}
              onChange={(value: Date) => formik.setFieldValue('last_inventory_check', value)}
            />
          </FormControl>
